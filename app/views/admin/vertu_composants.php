...
<head>
    ...
    <title>Composants — <?= htmlspecialchars($vertu['nom']) ?></title>
    ...
    <style>
        ...
        .vertu-resume-icon {
            ...
            background: rgba(107,180,107,0.1); display: flex;
            align-items: center; justify-content: center;
            flex-shrink: 0; color: #2a6a3a;
        }
        ...
        .global-save-bar .save-info { font-size: 0.85rem; color: var(--texte-light); margin-right: auto; }
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        ...
        <div class="page-header">
            <h2><?= icon('lien', 20) ?> Composants — <?= htmlspecialchars($vertu['nom']) ?></h2>
            <a href="<?= APP_URL ?>/admin/vertus" class="btn btn-secondary btn-sm">← Vertus</a>
        </div>
        ...
        <!-- Résumé vertu -->
        <div class="vertu-resume">
            <div class="vertu-resume-icon"><?= icon('vertu', 28) ?></div>
            <div>
                <strong style="font-family:var(--font-serif); font-size:1.1rem;">
                    <?= htmlspecialchars($vertu['nom']) ?>
                </strong>
            ...
            <?php $nb_c = count($composants_lies); ?>
            <div class="resume-tags">
                <span class="tag tag-composant"><?= $nb_c ?> composant<?= $nb_c > 1 ? 's' : '' ?> lié<?= $nb_c > 1 ? 's' : '' ?></span>
                <a href="<?= APP_URL ?>/admin/vertus/<?= $vertu['id'] ?>/editer"
        ...
        <div class="info-notice">
            <?= icon('composant', 15) ?>
            <span>
                Les composants cochés ici porteront cette vertu.
                Elle sera <strong>visible automatiquement</strong> sur toutes les plantes
                qui contiennent ces composants — fiches publiques, graphe, accueil.
            </span>
        </div>

        <!-- Formulaire unique -->
        <form method="POST"
              action="<?= APP_URL ?>/admin/vertus/<?= $vertu['id'] ?>/composants/bulk"
              id="form-composants">
            <input type="hidden" name="csrf_token" value="<?= $token ?>">

            <div class="liens-section">
                <h3><?= icon('composant', 16, 'section-icon composant') ?> Composants</h3>
                <p style="font-size:0.85rem; color:var(--texte-light); margin-bottom:0.75rem;">
                    Coche les composants qui portent la vertu <strong><?= htmlspecialchars($vertu['nom']) ?></strong>.
                </p>

                <?php $comp_ids = array_column($composants_lies, 'id'); ?>
                ...
                <div class="check-grid" id="grid-composants">
                    <?php foreach ($all_composants as $c): if (empty($c['nom'])) continue;
                        $checked = in_array($c['id'], $comp_ids); ?>
            ...
            <div class="global-save-bar">
                <span class="save-info">
                    Ces composants porteront la vertu "<?= htmlspecialchars($vertu['nom']) ?>" sur tout le site.
                </span>
                <button type="submit" class="btn btn-primary">
                    <?= icon('save', 17) ?> Enregistrer les composants
                </button>
            </div>
