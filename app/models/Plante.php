...
// Plante — modèle
// ================================================
// Gère les plantes et leurs liaisons avec :
//   - les catégories  (plante_categorie)
//   - les composants  (plante_composant)
//   - les vertus      (plante_vertu)

class Plante extends Model {
    protected string $table = 'plantes';

    // Liste toutes les plantes actives
    public function all(string $orderBy = 'nom', string $dir = 'ASC'): array {
        return $this->query(
            "SELECT * FROM plantes ORDER BY {$orderBy} {$dir}"
        );
    }

    // Trouve une plante par son slug
    ...
    public function update(int $id, array $data): void {
        ...
        $this->execute(
            "UPDATE plantes
             SET nom=?, nom_latin=?, slug=?, description=?, actif=?
             WHERE id=?",
            [
                trim($data['nom']),
                trim($data['nom_latin'] ?? ''),
    ...
    // Met à jour uniquement l'image
    public function updateImage(int $id, string $image): void {
        $this->execute(
            "UPDATE plantes SET image=? WHERE id=?",
            [$image, $id]
        );
    }
    ...
    public function linkCategorie(int $planteId, int $categorieId): void {
        $this->execute(
            "INSERT IGNORE INTO plante_categorie (plante_id, categorie_id)
             VALUES (?, ?)",
    ...
    public function unlinkCategorie(int $planteId, int $categorieId): void {
        $this->execute(
            "DELETE FROM plante_categorie
             WHERE plante_id=? AND categorie_id=?",
            [$planteId, $categorieId]
        );
    }
    ...
    public function linkComposant(int $planteId, int $composantId, string $concentration = '', string $notes = ''): void {
        $this->execute(
            "INSERT IGNORE INTO plante_composant
             (plante_id, composant_id, concentration, notes)
             VALUES (?, ?, ?, ?)",
    ...
    public function unlinkComposant(int $planteId, int $composantId): void {
        $this->execute(
            "DELETE FROM plante_composant
             WHERE plante_id=? AND composant_id=?",
    ...
    public function linkVertu(int $planteId, int $vertuId, string $source = ''): void {
        $this->execute(
            "INSERT IGNORE INTO plante_vertu
             (plante_id, vertu_id, source)
             VALUES (?, ?, ?)",
    ...
    public function unlinkVertu(int $planteId, int $vertuId): void {
        $this->execute(
            "DELETE FROM plante_vertu
             WHERE plante_id=? AND vertu_id=?",
            [$planteId, $vertuId]
        );
    }
}
